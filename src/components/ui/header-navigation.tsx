...
> {
  ...
  render() {
    return (
      <nav className={cn(this.props.isTransparent ? "relative z-50 text-white" : "bg-background text-foreground",
        this.props.isTransparent && this.state.showMenu ? "bg-background mb-[72px]" : ""
      )}>
        <Container className="px-8 py-4">
          <div className="flex flex-wrap items-center justify-between">
            <a
              href="/"
              className={cn(this.props.isTransparent && !this.state.showMenu ? "text-white" : "text-foreground", " flex items-center gap-3 hover:no-underline")}
            >
              <span className={cn(this.props.isTransparent && !this.state.showMenu ? "text-white" : "text-foreground", "self-center whitespace-nowrap text-xl font-black")}>
                1st Chertsey
              </span>

              <HeaderLogo isTransparent={this.props.isTransparent && !this.state.showMenu} />
            </a>
            <button
              id="toggle"
              type="button"
                    ...
                    {type === "link" ? (
                      links?.length ? (
                        <DropdownLink
                          ...
                          isTransparent={this.props.isTransparent && !this.state.showMenu}
                        />
                      ) : (
                        <Link
                          href={href}
...
> {
  ...
  render() {
    return (
      ...
      >
        <path
          className={cn(this.props.isTransparent ? "fill-white" : "fill-foreground")}
    ...
    return (
      <a
        className={cn(
          ...
          this.props.isTransparent ? "text-white" : "text-foreground hover:text-primary",
        )}
        href={this.props.href}
      >
        {this.props.text}
